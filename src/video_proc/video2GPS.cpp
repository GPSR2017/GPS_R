...
videoFrame parseVideoLine( string &line ) {
    ...
    if (broken_line.size() == 6) {
        setSucess = vid.set_values(broken_line[1], broken_line[2], broken_line[3], broken_line[4]);
...
void videoFrame::print() {
    cout << "Date: " << date << ", Time: " << time << ", Lat: " << lat << ", Lon:" << lon << endl;
}
 ...
 */
int videoFrame::set_values ( string d, string t, string latitude, string longitude ) {
    // Add guards here.
    bool error;
    error = (d.size() != 10 || t.size() != 8 || latitude.size() != 10 || longitude.size() != 10);
    if (error) {
...
bool videoFrame::checkVals() {
    ...
    valid = (regex_match(date, dateCheck) && regex_match(time, timeCheck) && regex_match(lat, latCheck) && regex_match(lon, lonCheck));
    return valid;
}
